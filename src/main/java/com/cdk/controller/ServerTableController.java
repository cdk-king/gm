...
public class ServerTableController {
    ...
    public Result addPlatform(@RequestBody Map map) {
        ...
        String sql = "insert into t_gameserver (server,serverIp,server_describe,platformId,sort,addUser,addDatetime,state,isDelete) " + " values ('" +
                server + "','" + serverIp + "','" + server_describe + "','" + platformId + "','0','" + addUser + "','" + addDatetime + "','0','0')";
        System.out.println("sql：" + sql);
    ...
    public Result editServer(@RequestBody Map map) {
        ...
        String sql = "UPDATE t_gameserver as a SET a.server='" + server + "',a.server_describe = '" + server_describe + "'," + "a.platformId='" +
                platformId + "'," + "a.serverIp='" + serverIp + "' ,a.addUser = '" + addUser + "' where a.id =" + id + "";
        System.out.println("sql：" + sql);
        Object args[] = new Object[]{id};
        int temp = jdbcTemplate.update(sql);
        if (temp > 0) {
