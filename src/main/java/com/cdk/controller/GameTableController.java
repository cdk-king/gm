...
public class GameTableController {
    ...
    public Result editGame(@RequestBody Map map) {
        ...
        String sql = "UPDATE t_game as a SET a.gameName='" + gameName + "',a.game_describe = '" + game_describe + "'," + "a.gameTag='" + gameTag +
                "' ,a.addUser = '" + addUser + "' where a.id =" + id + "";
        System.out.println("sql：" + sql);
    ...
    public Result addGame(@RequestBody Map map) {
        ...
        //describe是MySQL的关键字
        String sql =
                "insert into t_game (gameName,gameTag,game_describe,sort,addUser,addDatetime,state,isDelete) " + " values ('" + gameName + "','" +
                        gameTag + "','" + game_describe + "','0','" + addUser + "','" + addDatetime + "','0','0')";
        System.out.println("sql：" + sql);
        Object args[] = new Object[]{id};
        int temp = jdbcTemplate.update(sql);
        if (temp > 0) {
